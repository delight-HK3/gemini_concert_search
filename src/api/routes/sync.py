...
from core.external_database import get_external_db
from services.sync_service import SyncService
from api.schemas import SyncResponse, SyncResultResponse, ConcertSearchResultResponse
...
def run_sync(
    ...
    """가수 키워드 동기화 실행 (외부 DB → Gemini → 외부 DB)"""
    if not settings.EXTERNAL_DATABASE_URL:
        raise HTTPException(status_code=500, detail="EXTERNAL_DATABASE_URL is not configured")
